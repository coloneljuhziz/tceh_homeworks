...
class DoneCommand(BaseCommand):
    ...
    def perform(self, objects, *args, **kwargs):
        ...
        selection = None
        print()
        print('Input task number to mark it as done: ')
        print()
        for index, obj in enumerate(objects):
            print('{}: {}'.format(index, str(obj)))
        while True:
            try:
                selection = int(input_function('Input task number: '))
                objects[selection].done = True
                break
            except IndexError:
                print('Bad item number, try again.')

        print(str(objects[selection]) + ': Task marked as done')
...
class UndoneCommand(BaseCommand):
    ...
    def perform(self, objects, *args, **kwargs):
        ...
        selection = None
        print()
        print('Input task number to mark it as undone: ')
        print()
        for index, obj in enumerate(objects):
            print('{}: {}'.format(index, str(obj)))
        while True:
